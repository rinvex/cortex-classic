class Lang {
    ...
    constructor(options) {
        options = options || {};
        this.locale = options.locale || this._inferLocale();
     ...
     */
    choice(key, number, replacements, locale) {
     ...
     */
    _findMessageInTree(pathSegments, tree) {
    ...
    _testInterval(count, interval) {
        ...
        if (matches[2]) {
            ...
            var leftNumber = this._convertNumber(matches[2]);
            if (leftNumber === Infinity) {
                leftNumber = -Infinity;
            }
            var rightNumber = this._convertNumber(matches[3]);
